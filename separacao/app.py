    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if selected_model == "d6":
                separator = demucs.api.Separator(model="htdemucs_6s", progress=True, device='cpu', jobs=1)
            elif selected_model == "d4":
                separator = demucs.api.Separator(model="htdemucs_ft", progress=True, device='cpu', jobs=1)
            elif selected_model == "d2":
                separator = demucs.api.Separator(model="htdemucs", progress=True, device='cpu', jobs=1)
            else:
